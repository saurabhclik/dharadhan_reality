...
<div x-data="{
    ...
    x-on:keydown.esc.window="isOpen = false, openedWithKeyboard = false" {{ $attributes->whereStartsWith('x-on:') }}>
    @if ($label)
        <label for="{{ $name }}"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __($label) }} @if ($required)
    ...
    <div class="relative">
        <!-- Trigger button -->
        <button type="button" role="combobox" class="form-control-combobox" x-on:click="isOpen = !isOpen"
        ...
        <template x-if="multiple">
            <div>
                <template x-for="(value, index) in selectedOptions" x-bind:key="index">
                    <input type="hidden" x-bind:name="'{{ str_replace('[]', '', $name) }}[' + index + ']'"
                        x-bind:value="value" />
                </template>
            </div>
        </template>

        <input x-show="!multiple" name="{{ $name }}" type="hidden" x-ref="hiddenTextField"
            x-bind:value="selectedOption" @if ($required) required @endif />
            ...
            @if ($searchable)
                ...
                <div class="border-b border-gray-200 dark:border-gray-700 p-2">
                    <input type="text"
                        ...
                        placeholder="{{ __('Search...') }}" x-on:input="getFilteredOptions($el.value)"
                        x-ref="searchField" />
                </div>
            @endif
            ...
            <ul class="max-h-60 overflow-y-auto py-1">
                <template x-for="(item, index) in options" x-bind:key="item.value">
                    @if ($multiple)
                        <li role="option">
                            <label
                                class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-white/90 dark:hover:bg-gray-800 cursor-pointer"
                                ...
                                <input type="checkbox"
                                    ...
                                    x-bind:value="item.value" x-bind:id="'option_' + index"
                                    x-bind:checked="selectedOptions.includes(item.value)"
                                    x-on:change="handleOptionToggle(item.value, $el.checked)" tabindex="0" />
                    ...
                    @else
                        <li class="combobox-option px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-white/90 dark:hover:bg-gray-800 cursor-pointer flex items-center justify-between"
                            ...
                            x-on:keydown.enter="setSelectedOption(item)" x-bind:id="'option_' + index" tabindex="0">
                            <span x-bind:class="selectedOption == item.value ? 'font-medium' : ''"
                                x-text="item.label"></span>
                            <svg x-cloak x-show="selectedOption == item.value" xmlns="http://www.w3.org/2000/svg"
