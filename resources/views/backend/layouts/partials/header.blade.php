            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                <a class="flex items-center text-gray-700 dark:text-gray-300" href="#"
                    ...
                    <span class="mr-3 h-8 w-8 overflow-hidden rounded-full">
                        <img src="{{ ld_apply_filters('user_list_page_avatar_item', auth()->user()->photo ? getUserPhoto(auth()->user()) : auth()->user()->getGravatarUrl(40), auth()->user()) }}"
                            @if (auth()->user()->photo) style="max-width: none;" @endif alt="User" />
                    </span>
                </a>
