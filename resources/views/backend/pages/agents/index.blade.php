                ...
                <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @if (auth()->user()->can('agent.create'))
                            <a href="{{ route('admin.agents.create') }}" class="btn-primary flex items-center gap-2">
                                ...
                                <tr
                                    ...
                                    <td class="px-5 py-4 sm:px-6">
                                        <input type="checkbox"
                                            ...
                                            {{ !auth()->user()->canBeModified($user, 'agent.delete') ? 'disabled' : '' }}>
                                    </td>
                                    <td class="px-5 py-4 sm:px-6 flex items-center md:min-w-[200px]">
                                        <a data-tooltip-target="tooltip-user-{{ $user->id }}"
                                            href="{{ auth()->user()->canBeModified($user) ? route('admin.agents.edit', $user->id) : '#' }}"
                                            class="flex items-center">
                                            <img src="{{ ld_apply_filters('user_list_page_avatar_item', $user->photo ? getUserPhoto($user) : $user->getGravatarUrl(40), $user) }}"
                                        ...
                                        @if (auth()->user()->canBeModified($user))
                                            <div id="tooltip-user-{{ $user->id }}"
                                                href="{{ route('admin.agents.edit', $user->id) }}"
                                    ...
                                    <td class="px-5 py-4 sm:px-6">{{ ucfirst($user->status) }}</td>
                                    <td class="px-5 py-4 sm:px-6">
                                    ...
                                    <td class="px-5 py-4 sm:px-6 flex justify-center">
                                        <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                            @if (auth()->user()->canBeModified($user))
                                                <x-buttons.action-item :href="route('admin.agents.edit', $user->id)" icon="pencil"
                                                    :label="__('Edit')" />
                                            @endif
                                            
                                            @if (auth()->user()->canBeModified($user) && $user->pan_card_file)
                                                <x-buttons.action-item :href="asset('storage/' . $user->pan_card_file)" icon="download"
                                                    :label="__('Pan Card')" :download=true />                                    
                                            @endif

                                            @if (auth()->user()->canBeModified($user) && $user->aadhar_card_front_file)
                                                <x-buttons.action-item :href="asset('storage/' . $user->aadhar_card_front_file)" icon="download"
                                                    :label="__('Aadhar Front')" :download=true />                              
                                            @endif

                                            @if (auth()->user()->canBeModified($user) && $user->aadhar_card_back_file)
                                                <x-buttons.action-item :href="asset('storage/' . $user->aadhar_card_back_file)" icon="download"
                                                    :label="__('Aadhar Back')" :download=true />                              
                                            @endif
                                            
                                            @if (auth()->user()->canBeModified($user, 'agent.delete'))
                                                <div x-data="{ deleteModalOpen: false }">
                                                    ...
                                                    <x-modals.confirm-delete id="delete-modal-{{ $user->id }}"
                                                        ...
                                                        content="{{ __('Are you sure you want to delete this agent?') }}"
                                                        formId="delete-form-{{ $user->id }}"
                                                        formAction="{{ route('admin.agents.destroy', $user->id) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4">
                                        <p class="text-gray-500 dark:text-gray-300">{{ __('No agents found') }}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div x-show="bulkDeleteModalOpen"
                ...
                <div class="px-4 text-center">
                    <p class="text-gray-500 dark:text-gray-300">
                        {{ __('Are you sure you want to delete the selected agents?') }}
                        {{ __('This action cannot be undone.') }}
                    </p>
                </div>
                <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                    <form id="bulk-delete-form" action="{{ route('admin.agents.bulk-delete') }}" method="POST">
                        @method('DELETE')
                        @csrf
