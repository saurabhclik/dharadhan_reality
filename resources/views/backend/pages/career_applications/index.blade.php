                    ...
                    <table id="dataTable" class="w-full dark:text-gray-400">
                        ...
                        <tbody>
                            @forelse ($careerApplications as $careerApplication)
                                <tr
                                    class="{{ $loop->index + 1 != count($careerApplications) ? 'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                    <td class="px-5 py-4 sm:px-6">
                                        <input type="checkbox"
                                            ...
                                            value="{{ $careerApplication->id }}" x-model="selectedTasks">
                                    </td>
                                    <td class="px-5 py-4 sm:px-6">{{ ucfirst($careerApplication->name) }}</td>
                                    <td class="px-5 py-4 sm:px-6">{{ $careerApplication->email }}</td>
                                    <td class="px-5 py-4 sm:px-6">{{ $careerApplication->phone }}</td>
                                    <td class="px-5 py-4 sm:px-6">{{ $careerApplication->position }}</td>
                                    <td class="px-5 py-4 sm:px-6">{{ $careerApplication->message }}</td>
                                    <td class="px-5 py-4 sm:px-6">{{ $careerApplication->created_at->format('Y-m-d') }}</td>

                                    @php ld_apply_filters('user_list_page_table_row_before_action', '', $careerApplication) @endphp
                                    <td class="px-5 py-4 sm:px-6 flex justify-center">
                                        <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                            @can('career.application.view')
                                                <x-buttons.action-item :href="route('admin.career-applications.download', $careerApplication->id)" icon="pencil" :label="__('Download Resume')" />
                                            @endcan

                                            @can('career.application.delete')
                                                <div x-data="{ deleteModalOpen: false }">
                                                    <x-buttons.action-item type="modal-trigger" modal-target="deleteModalOpen"
                                                        icon="trash" :label="__('Delete')"
                                                        class="text-red-600 dark:text-red-400" />

                                                    <x-modals.confirm-delete id="delete-modal-{{ $careerApplication->id }}"
                                                        title="{{ __('Delete Career Application') }}"
                                                        content="{{ __('Are you sure you want to delete this career application?') }}"
                                                        formId="delete-form-{{ $careerApplication->id }}"
                                                        formAction="{{ route('admin.career-applications.destroy', $careerApplication->id) }}"
                                    ...
                                    @php ld_apply_filters('user_list_page_table_row_after_action', '', $careerApplication) @endphp
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4">
                                        <p class="text-gray-500 dark:text-gray-400">{{ __('No career applications found') }}
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div x-show="bulkDeleteModalOpen"
                    ...
                    <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2 dark:border-gray-800">
                        ...
                        <h3 id="bulk-delete-modal-title" class="font-semibold tracking-wide text-gray-800 dark:text-white">
                            {{ __('Delete Selected Career Applications') }}
                        </h3>
                    ...
                    <div class="px-4 text-center">
                        <p class="text-gray-500 dark:text-gray-400">
                            {{ __('Are you sure you want to delete the selected career applications?') }}
                            {{ __('This action cannot be undone.') }}
                        </p>
                    </div>
                    <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                        <form id="bulk-delete-form" action="{{ route('admin.career-applications.bulk-delete') }}" method="POST">
                            @method('DELETE')
                            @csrf
