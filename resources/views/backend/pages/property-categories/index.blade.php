            ...
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="px-5 py-4 sm:px-6 sm:py-5 flex gap-3 md:gap-1 flex-col md:flex-row justify-between items-center">
                    ...
                    <div class="flex items-center justify-end w-full md:w-auto gap-4">
                        ...
                        @can('property.category.create')
                            <a href="{{ route('admin.property.categories.create') }}" class="btn-primary ml-2">
                                <iconify-icon icon="feather:plus" class="mr-2"></iconify-icon>
                                {{ __('New Property Category') }}
                            </a>
                        @endcan
                    </div>
                </div>
                    ...
                    <table id="dataTable" class="w-full dark:text-gray-400">
                        <thead class="bg-light text-capitalize">
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <th width="20%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">
                                    <div class="flex items-center">
                                        {{ __('Slug') }}
                                        <a href="{{ request()->fullUrlWithQuery(['sort' => request()->sort === 'slug' ? '-slug' : 'slug']) }}"
                                            class="ml-1">
                        ...
                        <tbody>
                            @forelse ($propertyCategories as $category)
                                <tr
                                    class="{{ $loop->index + 1 != count($propertyCategories) ? 'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                    <td class="px-5 py-4 sm:px-6">
                                        <input type="checkbox"
                                            ...
                                            value="{{ $category->id }}" x-model="selectedTasks">
                                    </td>
                                    <td class="px-5 py-4 sm:px-6">
                                        <a data-tooltip-target="tooltip-user-{{ $category->id }}"
                                            href="{{ route('admin.property.categories.edit', $category->id) }}"
                                    ...
                                    <td class="px-5 py-4 sm:px-6">{{ ucfirst($category->status) }}</td>

                                    @php ld_apply_filters('user_list_page_table_row_before_action', '', $category) @endphp
                                    <td class="px-5 py-4 sm:px-6 flex justify-center">
                                        <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                            @can('property.category.edit')
                                                <x-buttons.action-item :href="route('admin.property.categories.edit', $category->id)" icon="pencil" :label="__('Edit')" />
                                            @endcan

                                            @can('property.category.delete')
                                                <div x-data="{ deleteModalOpen: false }">
                                                    <x-buttons.action-item type="modal-trigger" modal-target="deleteModalOpen"
                                                        icon="trash" :label="__('Delete')"
                                                        class="text-red-600 dark:text-red-400" />

                                                    <x-modals.confirm-delete id="delete-modal-{{ $category->id }}"
                                                        title="{{ __('Delete Property Category') }}"
                                                        content="{{ __('Are you sure you want to delete this property category?') }}"
                                                        formId="delete-form-{{ $category->id }}"
                                                        formAction="{{ route('admin.property.categories.destroy', $category->id) }}"
                                    ...
                                    @php ld_apply_filters('user_list_page_table_row_after_action', '', $category) @endphp
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4">
                                        <p class="text-gray-500 dark:text-gray-400">
                                            {{ __('No property categories found') }}
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div x-show="bulkDeleteModalOpen"
                    ...
                    <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2 dark:border-gray-800">
                        ...
                        <h3 id="bulk-delete-modal-title" class="font-semibold tracking-wide text-gray-800 dark:text-white">
                            {{ __('Delete Selected Property Categories') }}
                        </h3>
                    ...
                    <div class="px-4 text-center">
                        <p class="text-gray-500 dark:text-gray-400">
                            {{ __('Are you sure you want to delete the selected property categories?') }}
                            {{ __('This action cannot be undone.') }}
                        </p>
                    </div>
                    <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                        <form id="bulk-delete-form" action="{{ route('admin.property.categories.bulk-delete') }}"
                            method="POST">
                            @method('DELETE')
                            @csrf
