                    ...
                    <table id="dataTable" class="w-full dark:text-gray-400">
                        <thead class="bg-light text-capitalize">
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <th width="20%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">
                                    <div class="flex items-center">
                                        {{ __('Rating') }}
                                        <a href="{{ request()->fullUrlWithQuery(['sort' => request()->sort === 'rating' ? '-rating' : 'rating']) }}"
                                            class="ml-1">
                                ...
                                <th width="20%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">
                                    <div class="flex items-center">
                                        {{ __('Comment') }}
                                        <a href="{{ request()->fullUrlWithQuery(['sort' => request()->sort === 'comment' ? '-comment' : 'comment']) }}"
                                            class="ml-1">
                                ...
                                <th width="20%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">
                                    <div class="flex items-center">
                                        {{ __('Review Date') }}
                                        <a href="{{ request()->fullUrlWithQuery(['sort' => request()->sort === 'review_date' ? '-review_date' : 'review_date']) }}"
                                            class="ml-1">
                                            @if (request()->sort === 'review_date')
                                                <iconify-icon icon="lucide:sort-asc" class="text-primary"></iconify-icon>
                                            @elseif(request()->sort === '-review_date')
                                                <iconify-icon icon="lucide:sort-desc" class="text-primary"></iconify-icon>
                                            @else
                                                <iconify-icon icon="lucide:arrow-up-down"
                                                    class="text-gray-400"></iconify-icon>
                        ...
                        <tbody>
                            @forelse ($reviews as $review)
                                <tr
                                    class="{{ $loop->index + 1 != count($reviews) ? 'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                    <td class="px-5 py-4 sm:px-6">
                                        <input type="checkbox"
                                            ...
                                            value="{{ $review->id }}" x-model="selectedTasks">
                                    </td>
                                    <td class="px-5 py-4 sm:px-6">
                                        <a data-tooltip-target="tooltip-property-{{ $review->property->id }}"
                                            href="{{ route('admin.properties.edit', $review->property->id) }}"
                                        ...
                                        <div id="tooltip-property-{{ $review->property->id }}"
                                            href="{{ route('admin.properties.edit', $review->property->id) }}"
                                    ...
                                    <td class="px-5 py-4 sm:px-6">
                                        <a data-tooltip-target="tooltip-user-{{ $review->user->id }}"
                                            href="{{ route('admin.users.edit', $review->user->id) }}"
                                            class="flex items-center">
                                            <div class="flex flex-col">
                                                <span>{{ $review->user->name }}</span>
                                            </div>
                                        </a>
                                        <div id="tooltip-user-{{ $review->user->id }}"
                                            href="{{ route('admin.users.edit', $review->user->id) }}"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-md shadow-xs opacity-0 tooltip dark:bg-gray-700">
                                            {{ __('Edit User') }}
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                    ...
                                    <td class="px-5 py-4 sm:px-6"><x-star-rating :rating="$review->rating" /></td>
                                    <td class="px-5 py-4 sm:px-6">{{ ucfirst($review->status) }}</td>

                                    <td width="20%" class="px-5 py-4 sm:px-6">{{ $review->comment }}</td>
                                    <td width="20%" class="px-5 py-4 sm:px-6">
                                        {{ $review->review_date->format('Y-m-d') }}
                                    </td>

                                    @php ld_apply_filters('user_list_page_table_row_before_action', '', $review) @endphp
                                    <td class="px-5 py-4 sm:px-6 flex justify-center">
                                        <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                            @can('review.edit')
                                                <x-buttons.action-item :href="route('admin.reviews.edit', $review->id)" icon="pencil" :label="__('Edit')" />
                                            @endcan
                                            ...
                                            @can('review.delete')
                                                <div x-data="{ deleteModalOpen: false }">
                                                    ...
                                                    <x-modals.confirm-delete id="delete-modal-{{ $review->id }}"
                                                        title="{{ __('Delete Route Schedule') }}"
                                                        content="{{ __('Are you sure you want to delete this review?') }}"
                                                        formId="delete-form-{{ $review->id }}"
                                                        formAction="{{ route('admin.reviews.destroy', $review->id) }}"
                                    ...
                                    @php ld_apply_filters('user_list_page_table_row_after_action', '', $review) @endphp
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4">
                                        <p class="text-gray-500 dark:text-gray-400">{{ __('No reviews found') }}
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div x-show="bulkDeleteModalOpen"
                    ...
                    <div class="px-4 text-center">
                        <p class="text-gray-500 dark:text-gray-400">
                            {{ __('Are you sure you want to delete the selected reviews?') }}
                            {{ __('This action cannot be undone.') }}
                        </p>
                    </div>
                    <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                        <form id="bulk-delete-form" action="{{ route('admin.reviews.bulk-delete') }}" method="POST">
                            @method('DELETE')
                            @csrf
