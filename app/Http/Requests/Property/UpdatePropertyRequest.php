...
class UpdatePropertyRequest extends FormRequest
{
    ...
    {
        return $this->checkAuthorization(Auth::user(), ['property.edit']);
    }
    ...
    {
        $termId = $this->route('property');  // Get current property ID from route

        $currentYear = date('Y');

        $rules = [
            'property_category_id' => 'required|exists:property_categories,id',
            'title'                => 'required|string|max:255|unique:properties,title,' . $termId,
        ...
        return ld_apply_filters('property.update.validation.rules', $rules);
    }
