...
use App\Http\Requests\User\StoreAgentRequest;
use App\Http\Requests\User\UpdateAgentRequest;
use App\Models\User;
...
class AgentController extends Controller
{
    ...
    {
        $this->checkAuthorization(Auth::user(), ['agent.view']);
        ...
        return view('backend.pages.agents.index', [
            'users' => $this->agentService->getUsers($filters),
            'breadcrumbs' => [
                'title' => __('Agents'),
            ],
        ]);
    ...
    {
        $this->checkAuthorization(Auth::user(), ['agent.create']);

        ld_do_action('user_create_page_before');

        return view('backend.pages.agents.create', [
            'roles' => $this->rolesService->getRolesDropdown('Agent'),
            'breadcrumbs' => [
                'title' => __('New Agent'),
                'items' => [
                    [
    ...
    {
        $this->checkAuthorization(Auth::user(), ['agent.edit']);
        ...
        return view('backend.pages.agents.edit', [
            'user' => $user,
            'roles' => $this->rolesService->getRolesDropdown("Agent"),
            'breadcrumbs' => [
                'title' => __('Edit Agent'),
    ...
    public function update(UpdateAgentRequest $request, int $id): RedirectResponse
    {
        ...
        session()->flash('success', __('Agent has been updated.'));
    ...
    {
        $this->checkAuthorization(Auth::user(), ['agent.delete']);
        $user = $this->agentService->getUserById($id);
        ...
        session()->flash('success', __('A agent has been deleted.'));

        $this->storeActionLog(ActionType::DELETED, ['agent' => $user]);
    ...
    public function bulkDelete(Request $request): RedirectResponse
    {
        ...
        if (empty($ids)) {
            return redirect()->route('admin.agents.index')
                ->with('error', __('No agents selected for deletion'));
        }
        ...
        if (in_array(Auth::id(), $ids)) {
            ...
            session()->flash('error', __('You cannot delete your own account. Other selected agents will be processed.'));

            // If no agents left to delete after filtering out current user.
            if (empty($ids)) {
                return redirect()->route('admin.agents.index')
                    ->with('error', __('No agents were deleted.'));
            }
        }
        ...
        if ($deletedCount > 0) {
            session()->flash('success', __(':count agents deleted successfully', ['count' => $deletedCount]));
        } else {
            session()->flash('error', __('No agents were deleted. Selected agents may include protected accounts.'));
        }
